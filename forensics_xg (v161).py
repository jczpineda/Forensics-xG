...
print("✅ Libraries restored. Forensic tools calibrated.")
...
print("✅ Interface rebooted. v161.0 ready.")
...
#                         "isCross": is_cross,
#                         "isHome": (team_name == home_name) # Flag for orientation
#                     })
#                 except: continue
...
# 
#                         # BASIC FILTER (Normalized Data for Stats)
...
# 
#                         # --- STATS CALCULATION (Using Normalized Data) ---
...
# 
#                         # --- VISUALIZATION SETUP ---
#                         vis_options = ["Momentum Map", "Duel Map (Tackles Won/Lost)", "Game Control (Possession)", "Zone 14 & Half-Spaces", "Creator Map (Shot Assists)", "Passing Network (Structure)", "Pass Sonar (Radar)", "Impact Zone (Convex Hull)", "Attacking Zones (5 Lanes)", "Zone Invasions", "The Pulse (Shot Race)", "The Breakout (Progressive Carries)", "The Air Raid (Crossing Zones)", "Pass Map", "Shot Map (Ballistics)", "Defensive Actions", "Heatmap"]
#                         modules = st.multiselect("Evidence Layers", vis_options, default=["The Breakout (Progressive Carries)"], key=f"mod_{manager}")
# 
#                         # --- PREPARE VISUAL DATAFRAME (THE FLIP) ---
#                         # We create a COPY for visualization that flips coordinates for Away Team
#                         viz_df = plot_df.copy()
#                         is_home_team = viz_df['isHome'].iloc[0] if not viz_df.empty else False
# 
#                         if not is_home_team:
#                             # INVERT COORDINATES FOR AWAY TEAM (Attack Right -> Left)
#                             viz_df['x'] = 100 - viz_df['x']
#                             viz_df['y'] = 100 - viz_df['y']
#                             viz_df['endX'] = 100 - viz_df['endX']
#                             viz_df['endY'] = 100 - viz_df['endY']
#                             attack_label = "⬅️ Attacking Direction"
#                         else:
#                             # Home Team (Attack Left -> Right)
...
#                             st.subheader(f"📡 Pass Sonar: {sel_player}")
#                             # Use NORMALIZED plot_df for angles so East is always attacking
...
#                             pitch.draw(ax=ax)
# 
#                             # --- 4. THE BREAKOUT (PROGRESSIVE CARRIES) [DASHED & FLIPPED] ---
#                             if "The Breakout (Progressive Carries)" in pitch_modules:
#                                 # We need logic that works for FLIPPED data too.
#                                 # SIMPLER: Calculate distance on NORMALIZED (plot_df) data first, then match by Index.
#                                 prog_indices = plot_df[plot_df['endX'] > plot_df['x'] + 10].index
#                                 carries_viz = viz_df.loc[prog_indices]
# 
#                                 if not carries_viz.empty:
#                                     pitch.lines(carries_viz.x, carries_viz.y, carries_viz.endX, carries_viz.endY,
...
#                                 p_z14 = len(actions[(actions['x'].between(65,85)) & (actions['y'].between(37,63)) & (actions['Type']==1)])
#                                 d_z14 = len(actions[(actions['x'].between(65,85)) & (actions['y'].between(37,63)) & (actions['Type']==3)])
# 
#                                 # Define drawing coordinates (Flip if away)
#                                 z14_x, z14_y = 65, 37
#                                 lhs_x, lhs_y = 65, 20
#                                 rhs_x, rhs_y = 65, 63
#                                 if not is_home_team:
#                                     z14_x, lhs_x, rhs_x = 100 - 85, 100 - 85, 100 - 85
# 
#                                 r_z14 = mpatches.Rectangle((z14_x, z14_y), 20, 26, alpha=0.4, color='#ffd700', ec='white')
#                                 ax.add_patch(r_z14)
#                                 ax.text(z14_x+10, z14_y+13, f"Zone 14\nP:{p_z14} | D:{d_z14}", color='white', ha='center', va='center', fontweight='bold', fontsize=10)
# 
#                             if "Attacking Zones (5 Lanes)" in pitch_modules:
#                                 f3_acts = plot_df[(plot_df['x'] > 66.6) & (plot_df['Type'] == 1) & (plot_df['Outcome'] == 'Successful')]
#                                 total = len(f3_acts)
#                                 if total > 0:
#                                     c_lf = len(f3_acts[f3_acts['y'] <= 20])
#                                     c_lh = len(f3_acts[(f3_acts['y'] > 20) & (f3_acts['y'] <= 37)])
#                                     c_c  = len(f3_acts[(f3_acts['y'] > 37) & (f3_acts['y'] <= 63)])
#                                     c_rh = len(f3_acts[(f3_acts['y'] > 63) & (f3_acts['y'] <= 80)])
#                                     c_rf = len(f3_acts[f3_acts['y'] > 80])
# 
#                                     start_x = 66.6 if is_home_team else 0
# 
#                                     # Lane 1 (Bottom/Left)
#                                     pct = (c_lf/total)*100
#                                     rect = mpatches.Rectangle((start_x, 0), 33.4, 20, alpha=min(0.9, max(0.2, pct/40)), color='#ff4b4b', ec='white')
#                                     ax.add_patch(rect)
#                                     # Lane 2
#                                     pct = (c_lh/total)*100
#                                     rect = mpatches.Rectangle((start_x, 20), 33.4, 17, alpha=min(0.9, max(0.2, pct/40)), color='#ff4b4b', ec='white')
#                                     ax.add_patch(rect)
#                                     # Lane 3 (Center)
#                                     pct = (c_c/total)*100
#                                     rect = mpatches.Rectangle((start_x, 37), 33.4, 26, alpha=min(0.9, max(0.2, pct/40)), color='#ff4b4b', ec='white')
#                                     ax.add_patch(rect)
#                                     # Lane 4
#                                     pct = (c_rh/total)*100
#                                     rect = mpatches.Rectangle((start_x, 63), 33.4, 17, alpha=min(0.9, max(0.2, pct/40)), color='#ff4b4b', ec='white')
#                                     ax.add_patch(rect)
#                                     # Lane 5 (Top/Right)
#                                     pct = (c_rf/total)*100
#                                     rect = mpatches.Rectangle((start_x, 80), 33.4, 20, alpha=min(0.9, max(0.2, pct/40)), color='#ff4b4b', ec='white')
#                                     ax.add_patch(rect)
# 
